...
var BackendCalendar = {
    ...
    initialize: function(defaultEventHandlers) {
        ...
        if (GlobalVariables.user.role_slug == Backend.DB_SLUG_WRITER) {
            $('#select-filter-item optgroup:eq(1)').remove();
        }
        
        if (GlobalVariables.user.role_slug == Backend.DB_SLUG_WRITER) {
            // Remove the providers that are not connected to the writer.
            $('#select-filter-item option[type="provider"]').each(function(index, option) {
                var found = false;
                $.each(GlobalVariables.writerProviders, function(index, id) {
                    if ($(option).val() == id) {
                        found = true;
                        return false;
                    }
        ...
        if (GlobalVariables.editAppointment != null) {
            ...
            $dialog.find('#select-writer').val(appointment['id_users_writer']);
    ...
    bindEventHandlers: function() {
        ...
        $('#manage-appointment #save-appointment').click(function() {
            ...
            var appointment = {
                ...
                'id_users_writer': $dialog.find('#select-writer').val(),
