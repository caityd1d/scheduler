 ...
 * classes (defined below) in order to handle the admin, provider and writer record types. 
 * 
...
var BackendUsers = {
    ...
    initialize: function(defaultEventHandlers) {
        ...
        $('#filter-writers .results').jScrollPane();
        ...
        $.each(GlobalVariables.providers, function(index, provider) {
           var html = '<label class="checkbox"><input type="checkbox" data-id="' + provider.id + '" />' 
            ...
            $('#writer-providers').append(html);
        });
        $('#writer-providers').jScrollPane({ mouseWheelSpeed: 70 });
    ...
    bindEventHandlers: function() {
        ...
        $('.tab').click(function() {
            ...
            if ($(this).hasClass('admins-tab')) { // display admins tab
                ...
                $.post(postUrl, postData, function(response) {
                    ...
                    $('#writer-providers').data('jsp').destroy();
                    $('#writer-providers').html('');
                    $.each(GlobalVariables.providers, function(index, provider) {
                        var html = '<label class="checkbox"><input type="checkbox" data-id="' + provider.id + '" />' 
                                + provider.first_name + ' ' + provider.last_name + '</label>';
                         $('#writer-providers').append(html);
        ...
        /**
         * Event: Admin, Provider, writer Username "Focusout" 
         * 
         ...
         */
        $('#admin-username, #provider-username, #writer-username').focusout(function() {
            var $input = $(this);
