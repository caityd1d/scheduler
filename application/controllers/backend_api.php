...
class Backend_api extends CI_Controller {
    ...
    /**
     * [AJAX] Filter writer records with string key.
     * 
     * @param string $_POST['key'] The key string used to filter the records.
     * @return array Returns a json encoded array back to client with the writer records.
     */
    public function ajax_filter_writers() {
        try {
            if ($this->privileges[PRIV_USERS]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $where = 
                ...
                'OR city LIKE "%' . $key . '%" OR state LIKE "%' . $key . '%" ' .
                'OR zip_code LIKE "%' . $key . '%" OR notes LIKE "%' . $key . '%")';
            $writers = $this->writers_model->get_batch($where);
    ...
    /**
     * [AJAX] Save (insert or update) a writer record into database.
     * 
     * @param array $_POST['writer'] A json encoded array that contains the writer data. 
     * If an 'id' value is provided then the record is going to be updated.
     * @return string Returns the success contant 'AJAX_SUCCESS' so javascript knows that  
     * everything completed successfully.
     */
    public function ajax_save_writer() {
        try {
            $this->load->model('writers_model');
            $writer = json_decode($_POST['writer'], true);
            
            $REQUIRED_PRIV = (!isset($writer['id'])) 
                    ? $this->privileges[PRIV_USERS]['add']
                    : $this->privileges[PRIV_USERS]['edit'];
            if ($REQUIRED_PRIV == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            ...
            $writer_id = $this->writers_model->add($writer);
    ...
    /**
     * [AJAX] Delete a writer record from the database.
     * 
     * @param numeric $_POST['writer_id'] The id of the record to be deleted.
     * @return string Returns the operation result constant (AJAX_SUCESS or AJAX_FAILURE).
     */
    public function ajax_delete_writer() {
        try {
            if ($this->privileges[PRIV_USERS]['delete'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $this->load->model('writers_model');
            $result = $this->writers_model->delete($_POST['writer_id']);
            echo ($result) ? json_encode(AJAX_SUCCESS) : json_encode(AJAX_FAILURE);
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
    ...
    public function ajax_validate_username() {
        try {            
            ...
            // for the rest user types for now (providers, writers).
            $this->load->model('admins_model');
